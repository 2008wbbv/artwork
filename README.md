...
- **The tape** *(an experiment)*. Instead of a painting, put a YouTube build timelapse behind the clock and it runs at whatever speed makes it end exactly when your interval does — two hours of somebody laying blocks compressed into twenty-five minutes of you. YouTube's player only offers rates up to 2×, nowhere near enough, so the rate does the fine work and the playhead is walked forward to keep the video on the same clock as the timer. Muted throughout, so whatever you're listening to carries on. There's a small fellow in that section swinging a pick at a block that never quite breaks.
...
Four of the ten have hung in the game since long before 2022, and the label says which: *Bonjour Monsieur Courbet* → **Courbet**, *Wanderer* → **Wanderer**, *The Stage Is Set* → **Stage**, *Graham* → **Graham**. That last one catches people out — the in-game *Graham* is not a portrait, it's a Sánchez Cotán still life with King Graham of *King's Quest* standing on the windowsill. The matching, and each canvas's real medium, date and physical size, come from [the Minecraft Wiki's painting table](https://minecraft.wiki/w/Painting) rather than from memory. The other six are paintings of his that never became one, and claim nothing.

Only those four of the classic twenty-six are on Commons under a free licence. The rest of the originals live on [the painter's own site](https://zetterstrand.com), which is not a free licence, so they aren't here.

The licence wants attribution, so the licence and a link to the file page go on the label beside the picture rather than in a credits file somewhere. The game's own textures and skins are not here and won't be: they are Mojang's files, and this is a public repository.
